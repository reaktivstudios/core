jQuery(document).ready(function ($) {
  ...
  $('body').append('<div id="rkv-admin-notices-overlay"></div>');


  // Close panel when clicking overlay
  $('#rkv-admin-notices-overlay').on('click', function () {
    togglePanel();
  });
  ...
  $(document).on('keyup', function (e) {
    if (e.key === 'Escape') {
      if ($('#rkv-admin-notices-panel').hasClass('open')) {
        togglePanel();
      }
    }
  });
  ...
  // Toggle panel
  $(document).on('click', '#wp-admin-bar-rkv-admin-notices-panel a', function (e) {
    e.preventDefault();
    togglePanel();
  });
  ...
  $(document).on('click', '.rkv-panel-notice-item .rkv-notice-action a, .rkv-shown-notice-item .rkv-notice-action a', function (e) {
    e.preventDefault();
    let button = $(this);
    let action_type = button.hasClass('whitelist') ? 'whitelist' : 'blacklist';
    let action_option = button.hasClass('undo') ? 'undo' : 'default';
    let notice_item = button.closest('.rkv-panel-notice-item');
    ...
    if (action_type == 'whitelist') {
      if (action_option == 'undo') {
        ...
        notice_item.find('.rkv-notice-action .whitelist').addClass('undo');
        notice_item.find('.rkv-notice-action .whitelist').html(rkvAdminNoticesData.remove_whitelist_label);
        notice_item.addClass('whitelisted-notices-item hidden-element');
        var new_item = notice_item.clone().children();
        new_item.addClass('rkv-shown-notice-item');
        new_item.attr('data-notice-id', notice_id);
        $('.rkv-admin-notices-selector').prepend(new_item);
        notice_item.remove();
      }
    } else {
      if (action_option == 'undo') {
        notice_item.find('.rkv-notice-action .blacklist').removeClass('undo');
        notice_item.find('.rkv-notice-action .blacklist').html(rkvAdminNoticesData.blacklist_label);
        notice_item.addClass('active-notices-item hidden-element');
      } else {
        notice_item.find('.rkv-notice-action .blacklist').addClass('undo');
        notice_item.find('.rkv-notice-action .blacklist').html(rkvAdminNoticesData.remove_blacklist_label);
    ...
    $.post(ajaxurl, {
      'action': 'rkv_admin_notice_action',
      'nonce': rkvAdminNoticesData.nonce,
  ...
  function updateNoticePanel() {
    ...
    const remove_types = rkvAdminNoticesData.admin_notice_options.notice_type_remove ? rkvAdminNoticesData.admin_notice_options.notice_type_remove : [];
    const panel_types = rkvAdminNoticesData.admin_notice_options.notice_type_display ? rkvAdminNoticesData.admin_notice_options.notice_type_display : [];
    const whitelist_notices = rkvAdminNoticesData.admin_notice_data.whitelist_notices ? rkvAdminNoticesData.admin_notice_data.whitelist_notices : [];
    const blacklist_notices = rkvAdminNoticesData.admin_notice_data.blacklist_notices ? rkvAdminNoticesData.admin_notice_data.blacklist_notices : [];

    if (notices.length === 0 || remove_types == '' || remove_types.length === 0) {
      // show our wrapped notices
      $('.rkv-admin-notices-selector').show();
    ...
    if (!initialize_notice) {
      ...
      if (active_notices.length > 0) {
        ...
        $.each(active_notices, function (index, element) {
          ...
          if (panel_notice_ids.includes(notice_id)) {
            ...
            {
              let notice_action_html = '<div class="rkv-notice-action">';
              notice_action_html += '<div class="action-item-wrap"><div class="rkv-tool-tip down-notice"><div class="dashicons dashicons-editor-help"></div><div class="tool-tip-text"><p>' + rkvAdminNoticesData.whitelist_note + '</p><i></i></div></div><a href="#" class="whitelist">' + rkvAdminNoticesData.whitelist_label + '</a></div>';
              notice_action_html += '<div class="action-item-wrap"><div class="rkv-tool-tip down-notice"><div class="dashicons dashicons-editor-help"></div><div class="tool-tip-text"><p>' + rkvAdminNoticesData.blacklist_note + '</p><i></i></div></div><a href="#" class="blacklist">' + rkvAdminNoticesData.blacklist_label + '</a></div>';
              notice_action_html += '</div>';
              $(element).append(notice_action_html);
            }
            notice_html += '<div class="rkv-panel-notice-item active-notices-item ' + additional_class + '" data-notice-id="' + notice_id + '">';
      ...
      if (whitelisted_notices.length > 0) {
        $.each(whitelisted_notices, function (index, element) {
          ...
          {
            let notice_action_html = '<div class="rkv-notice-action">';
            notice_action_html += '<div class="action-item-wrap"><div class="rkv-tool-tip down-notice"><div class="dashicons dashicons-editor-help"></div><div class="tool-tip-text"><p>' + rkvAdminNoticesData.whitelist_note + '</p><i></i></div></div><a href="#" class="whitelist undo">' + rkvAdminNoticesData.remove_whitelist_label + '</a></div>';
            notice_action_html += '<div class="action-item-wrap"><div class="rkv-tool-tip down-notice"><div class="dashicons dashicons-editor-help"></div><div class="tool-tip-text"><p>' + rkvAdminNoticesData.blacklist_note + '</p><i></i></div></div><a href="#" class="blacklist">' + rkvAdminNoticesData.blacklist_label + '</a></div>';
            notice_action_html += '</div>';
            $(element).addClass('rkv-shown-notice-item').attr('data-notice-id', notice_id);
            $(element).append(notice_action_html);
          }
        });
      }
      ...
      if (blacklisted_notices.length > 0) {
        ...
        $.each(blacklisted_notices, function (index, element) {
          ...
          //if ($(element).is('.notice'))
          {
            let notice_action_html = '<div class="rkv-notice-action">';
            notice_action_html += '<div class="action-item-wrap"><div class="rkv-tool-tip down-notice"><div class="dashicons dashicons-editor-help"></div><div class="tool-tip-text"><p>' + rkvAdminNoticesData.whitelist_note + '</p><i></i></div></div><a href="#" class="whitelist">' + rkvAdminNoticesData.whitelist_label + '</a></div>';
            notice_action_html += '<div class="action-item-wrap"><div class="rkv-tool-tip down-notice"><div class="dashicons dashicons-editor-help"></div><div class="tool-tip-text"><p>' + rkvAdminNoticesData.blacklist_note + '</p><i></i></div></div><a href="#" class="blacklist undo">' + rkvAdminNoticesData.remove_blacklist_label + '</a></div>';
            notice_action_html += '</div>';
            $(element).append(notice_action_html);
          }
          notice_html += '<div class="rkv-panel-notice-item blacklisted-notices-item ' + additional_class + '" data-notice-id="' + notice_id + '">';
          notice_html += $(element).prop('outerHTML')
          notice_html += '</div>';
          blacklisted_notices_count++;
          $(element).remove();
  ...
  function updateAdminNoticesCounts(success_count = false, error_count = false, warning_count = false, info_count = false, active_notices_count = false, blacklisted_notices_count = false) {
    ...
    if (!success_count) {
      success_count = $('.rkv-panel-notice-item.active-notices-item .notice-success, .rkv-panel-notice-item.active-notices-item .updated').length;
    }
    if (!error_count) {
      error_count = $('.rkv-panel-notice-item.active-notices-item .notice-error, .rkv-panel-notice-item.active-notices-item .error').length;
    }
    if (!warning_count) {
      warning_count = $('.rkv-panel-notice-item.active-notices-item .notice-warning, .rkv-panel-notice-item.active-notices-item .warning').length;
    }
    if (!info_count) {
      info_count = $('.rkv-panel-notice-item.active-notices-item .notice-info, .rkv-panel-notice-item.active-notices-item .info').length;
    }
    ...
    $('#wp-admin-bar-rkv-admin-notices-panel .rkv-admin-notices-count').html(success_count_html + error_count_html + warning_count_html + info_count_html);
    if (success_count > 0) {
      $('.rkv-admin-notices-count .success-counter').show();
    } else {
      $('.rkv-admin-notices-count .success-counter').hide();
    }
    if (error_count > 0) {
      $('.rkv-admin-notices-count .error-counter').show();
    } else {
      $('.rkv-admin-notices-count .error-counter').hide();
    }
    if (warning_count > 0) {
      $('.rkv-admin-notices-count .warning-counter').show();
    } else {
      $('.rkv-admin-notices-count .warning-counter').hide();
    }
    if (info_count > 0) {
      $('.rkv-admin-notices-count .info-counter').show();
    } else {
      $('.rkv-admin-notices-count .info-counter').hide();
    }

    // update tab count/display
    var active_notices_counter = active_notices_count;
    if (!active_notices_count) {
      active_notices_count = $('.rkv-panel-notice-item.active-notices-item').length;
    ...
    if (!blacklisted_notices_count) {
      blacklisted_notices_count = $('.rkv-panel-notice-item.blacklisted-notices-item').length;
   ...
   // $('.admin-notices-tab label').removeClass('selected');
    ...
    if (tab_counts > 1 || tab_counts > 0 && active_notices_count === 0) {
      ...
      $('.rkv-admin-notices-panel-none').hide();
    } else if (tab_counts === 0) {
      $('.rkv-admin-notices-panel-none').show();
    } else {
      $('.rkv-admin-notices-panel-none').hide();
    }
  ...
  function togglePanel() {
    const $panel = $('#rkv-admin-notices-panel');
    const $overlay = $('#rkv-admin-notices-overlay');
    const $counter = $('#wp-admin-bar-rkv-admin-notices-panel .rkv-admin-notices-count');
    ...
    if ($panel.hasClass('open')) {
      ...
      if (!initialize_notice) {
        if (notice_html && notice_html !== '') {
          $('#rkv-admin-notices-panel .rkv-admin-notices-panel-content').html(notice_html);
          $('.rkv-admin-notices-panel-none').hide();
          updateAdminNoticesCounts();
        }
